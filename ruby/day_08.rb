...
def parse_input(fname)
  ...
  File.readlines(fname).map(&:chomp).each do |line|
    ...
    signals = left.split.map { |sig| sig.chars.sort.join }
    digits = right.split.map { |dgt| dgt.chars.sort.join }
...
def find_wiring(bins)
  ...
  wiring[2] = find_two(bins, wiring[9])
...
def find_six(bins)
  ...
  bins[SIX_SEGMENTS].reject { |s| (one.chars - s.chars).empty? }.first
end

def find_nine(bins)
  four = bins[FOUR_SEGMENTS].first
  bins[SIX_SEGMENTS].filter { |s| (four.chars - s.chars).empty? }.first
end

def find_three(bins)
  one = bins[TWO_SEGMENTS].first
  bins[FIVE_SEGMENTS].filter { |s| (one.chars - s.chars).empty? }.first
end

def find_two(bins, nine)
  bins[FIVE_SEGMENTS].reject { |s| (s.chars - nine.chars).empty? }.first
end
...
def part_one(displays)
  displays.sum { |display| display.digits.count { |d| [2, 3, 4, 7].include? d.size } }
end
